...
import { MessageCircle, Search, Settings2, ShoppingBag, X } from "lucide-react";
...
export default function HomePage() {
  ...
  useEffect(() => {
    try {
      ...
      if (savedPhone?.trim()) {
        ...
        localStorage.setItem(WHATSAPP_PHONE_STORAGE_KEY, "9647804000463");
      }
    } catch {
    ...
    return groups
      .map((group) => ({
        ...
        items: group.items.filter((item) => {
          const target = normalizeSearch(`${item.name} ${item.description} ${item.notes ?? ""}`);
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      ...
      { rootMargin: "-32% 0px -50% 0px", threshold: [0.18, 0.35, 0.55] }
    );
  ...
  return (
    <main className="min-h-screen overflow-hidden bg-luxury-black text-white">
      ...
      <section id="menu" className="relative px-4 py-14 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 grid gap-5 lg:grid-cols-[1fr_26rem] lg:items-end">
            ...
            <div className="flex gap-3">
              <label className="relative block flex-1">
                <Search className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-white/42" size={20} />
                <input
                  value={searchTerm}
        ...
        <CategoryTabs categories={categories} activeCategory={activeCategory} onSelect={scrollToCategory} />

        <div className="mx-auto mt-10 max-w-7xl">
          {hasSearch ? (
            <p className="mb-6 rounded-2xl border border-white/10 bg-white/[0.045] px-4 py-3 text-sm font-bold text-white/62">
              نتائج البحث: <span className="text-luxury-mint">{resultCount}</span>
            </p>
          ) : null}
          {filteredGroups.length === 0 ? (
            <div className="glass-panel mx-auto grid max-w-lg place-items-center rounded-[2rem] px-6 py-16 text-center">
              ...
              <p className="mt-3 leading-7 text-white/58">لا يوجد طبق مطابق حالياً في منيو الربيع.</p>
            </div>
          ) : (
              ...
              {filteredGroups.map((group) => (
                ...
                >
                  <div className="relative mb-6 min-h-[22rem] overflow-hidden rounded-[2rem] border border-white/10 shadow-soft-lift sm:min-h-[28rem]">
                    <Image src={group.category.image} alt={group.category.title} fill sizes="(min-width: 1024px) 1200px, 100vw" className="object-cover transition duration-700 hover:scale-105" />
                    <div className="absolute inset-0 bg-gradient-to-t from-black via-black/55 to-transparent" />
                    <div className="absolute inset-x-0 bottom-0 p-5 sm:p-8">
                      <p className="mb-3 text-xs font-black uppercase tracking-[0.32em] text-luxury-mint">{group.category.subtitle}</p>
                      <h3 className="text-4xl font-black text-white sm:text-6xl">{group.category.title}</h3>
                      <p className="mt-4 max-w-2xl text-sm leading-7 text-white/68 sm:text-base sm:leading-8">{group.category.description}</p>
                    </div>
                  </div>
      ...
      <div className="fixed inset-x-4 bottom-4 z-40 flex items-center justify-between gap-3 sm:hidden">
        <button type="button" onClick={() => setIsCartOpen(true)} className="relative inline-flex min-h-14 flex-1 items-center justify-center gap-3 rounded-2xl border border-white/12 bg-white/12 px-5 py-3 text-sm font-black text-white shadow-soft-lift backdrop-blur-2xl transition active:scale-[0.98]">
          <ShoppingBag size={19} />
          السلة
          {cartCount > 0 ? <span className="grid h-6 min-w-6 place-items-center rounded-full bg-luxury-mint px-1 text-xs text-luxury-black">{cartCount}</span> : null}
        </button>
        <button type="button" onClick={() => setIsCartOpen(true)} className="grid h-14 w-14 place-items-center rounded-2xl bg-luxury-mint text-luxury-black shadow-green-glow transition active:scale-[0.98]" aria-label="طلب عبر واتساب">
          <MessageCircle size={22} />
        </button>
      </div>

      <a
        href={`${restaurantInfo.whatsappBaseUrl}/${whatsappPhone}?text=${encodeURIComponent("مرحباً مطعم الربيع، أريد الاستفسار عن المنيو.")}`}
        target="_blank"
        rel="noreferrer"
